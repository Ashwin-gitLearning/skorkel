   ...
   protected void BinduserSearch()
    {
        if (Convert.ToString(Session["ExternalUserId"]) != "" && Session["ExternalUserId"] != null)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ViewState["MaxCount"] = Convert.ToString(dt.Rows[0]["Maxcount"]);
                hdnMaxcount.Value = Convert.ToString(dt.Rows[0]["Maxcount"]);
                
               if (Convert.ToInt32(dt.Rows[0]["Maxcount"]) == 1) {
                    lblTotalMember.Text = "1 Member";
                } else
                {
                    lblTotalMember.Text = Convert.ToString(dt.Rows[0]["Maxcount"]) + " Members";
                }
    ...
    protected void lstPostUpdates_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        HtmlGenericControl tbl = (HtmlGenericControl)e.Item.FindControl("tbl");
    ...
    {
        int NextPage = 12 + Convert.ToInt32(hdnTotalItem.Value);
        int count = 0;

       

        if (Convert.ToInt32(hdnTotalItem.Value) <= Convert.ToInt32(ViewState["MaxCount"]) )
        {
    ...
    protected void lnkPopupOK_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (ISAPIURLACCESSED == "1")
            {
				...
				UserURL.Append("massageToUser.action?");
				UserURL.Append("messageByUserId=USR");
				UserURL.Append(ViewState["messageByUserId"]);
				UserURL.Append("&messageToUserId=USR");
				UserURL.Append(ViewState["intRegistrationId"]);
				UserURL.Append("&message=");
				UserURL.Append(WallMessageDO.StrRecommendation);
        ...
        ScriptManager.RegisterStartupScript(this, this.GetType(), "starSts1", "hideScroll();", true);
        Clear();
        //Response.Redirect("group-event-main.aspx?GrpId=" + ViewState["intGroupId"]);
            ...
            if (dtSearch.Rows.Count > 0)
            {
                ...
                if (Convert.ToString(Session["ExternalUserId"]) != "" && Session["ExternalUserId"] != null)
                {
                    ...
                    try
                    {
						...
						UserURL.Append("&members=USR");
						UserURL.Append(ViewState["AccRegId"]);
            ...
            if (dtSearch.Rows.Count > 0)
            {
                ...
                if (Convert.ToString(Session["ExternalUserId"]) != "" && Session["ExternalUserId"] != null)
                {
                    ...
                    try
                    {
						...
						StringBuilder UserURL = new StringBuilder();
						UserURL.Append(APIURL);
						UserURL.Append("removeMemberFromGroup.action?");
						UserURL.Append("groupId=GRP");
						UserURL.Append(Request.QueryString["GrpId"]);
						UserURL.Append("&members=USR");
						UserURL.Append(ViewState["RejRegId"]);
    ...
    private void SendGroupMail(string mailid, string name, string groupname)
    {
        try
        {
            ...
            //    clientip.EnableSsl = true;
            string DisplayName = ConfigurationManager.AppSettings["DisplayName"];
            string MailUrlFinal = MailURL + "/Group-Profile.aspx?GrpId=" + ViewState["intGroupId"];
            try
            {
                MailMessage Rmm2 = new MailMessage();
                Rmm2.IsBodyHtml = true;
                ...
                body = body.Replace("{Content}", "You are a now a member of " + "<span style='font-weight:bold;font-size:14px;color:#373A36;'>" + groupname + "</span>." +
                ...
                Rmm2.Subject = "Request to Join " + groupname + " Accepted";
                Rmm2.To.Clear();
                Rmm2.To.Add(MailTo);
                //Rmm2.Subject = "Skorkel group joining invitation";
                ...
                FCMNotification.Send("Request to Join " + groupname + " Accepted", "You are a now a member of " + groupname + ".",
