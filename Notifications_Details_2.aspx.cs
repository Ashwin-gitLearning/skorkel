...
public partial class Notifications_Details_2 : System.Web.UI.Page
{
    ...
    protected void lstMainMyTag_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        Panel pnlDateAdded = (Panel)e.Item.FindControl("pnlDateAdded");
        objdonetwork.NotificationDate =Convert.ToString(lblAddedOn.Text.Trim());
        objdonetwork.RegistrationId = Convert.ToInt32(Session["ExternalUserId"]); 

        
            pnlDateAdded.Visible = true;
    ...
    protected void lstChildMyTag_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        if (imgIcon.Src == "" || imgIcon.Src == null || imgIcon.Src == "CroppedPhoto/" || imgIcon.Src != "")
        { 
             string imgPathPhysical = Server.MapPath("~/CroppedPhoto/" + imgIcon.Src);
            if (File.Exists(imgPathPhysical))
            {
                imgIcon.Src = "~\\CroppedPhoto\\" + imgIcon.Src;
            }
            else
            {
                imgIcon.Src = "images/profile-photo.png";
            }
        }
        ...
        if (hdnTableName.Value == "SCL_SA_Notifications")
        {
         
            lnkName.Text = "Skorkel";
            lblnotificationname.Text = "Notification";
            lnkstrLink.Text = lblGroupName.Text;
            lnkstrLink.Attributes["class"] = "remove-hover-anchor";
            lnkConfirm.Style.Add("display", "none");
            lnkCancel.Style.Add("display", "none");
            imgIcon.Src = "images/notificaion-logo.png";
