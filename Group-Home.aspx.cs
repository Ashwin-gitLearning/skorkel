...
public partial class Group_Home : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            //HtmlGenericControl bodyelement = (HtmlGenericControl)Master.FindControl("bodyelement");
            masterlbl.InnerText = "Groups";
            //bodyelement.Attributes.Add("class", "active-toogle on-flag");
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        //myObject = (HtmlContainerControl)Master.FindControl("bodyelement");
    }
    ...
    protected void btnPostUpdate_Click(object sender, EventArgs e)
    {
        ...
        if (SavePost())
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ModalClose", "ClearOrgText();$('#wallModal').modal('hide');", true);
        }
    }
    ...
    protected bool SavePost()
    {
        try
        {
            if (Session["SubmitTime"].ToString() == ViewState["SubmitTime"].ToString())
            {
                ...
                string editor = txtPostUpdate.InnerText.Trim().Replace("<p>", "<br>").Replace("</p>", "");
                string noHTML = Regex.Replace(editor, @"<[^>]+>|&nbsp;", "").Trim();

                objGrpstatusDO.strPostDescription = Regex.Replace(noHTML, @"\s{2,}", " ");
                //txtPostUpdate.InnerHtml.Trim().Replace("'", "''");
                ...
                {
                    lblMessage.Text = "Media updated successfully.";
                }
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "showSuccess", "showGrpHomeSuccesPopup();", true);
    ...
    protected void lstPostUpdates_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        HiddenField hdnlnkLike = (HiddenField)e.Item.FindControl("hdnlnkLike");
        //HtmlGenericControl bodyelement = (HtmlGenericControl)e.Item.FindControl("ctl00_bodyelement");
            ...
            try
            {
                if (Session["SubmitTime"].ToString() == ViewState["SubmitTime"].ToString())
                {
                    ...
                    if (txtComment.Text != "" && txtComment.Text != "Write a comment" && txtComment.Text != null)
                    {
                        ...
                        {
                            //myObject.Add("class", "active-toogle on-flag");   
                            //HtmlGenericControl bodyelement = this.Master.FindControl("ctl00_bodyelement") as HtmlGenericControl;
                            //bodyelement.Attributes.Add("class", "active-toogle on-flag");
        ...
        if (e.CommandName == "Hide/ShowComments")
        {
            if (lstChild.Visible == true)
            {
                ...
                imgUpDown.ImageUrl = "images/chevron-down.svg";
            }
            else
            {
                lstChild.Visible = true;
                imgUpDown.ImageUrl = "images/chevron-up.svg";
            }
        }
        ...
        if (e.CommandName == "Edit Post")
        {
            ...
            if (dt.Rows.Count > 0)
            {
               string strPostDesc = Convert.ToString(dt.Rows[0]["strPostDescription"]).Replace("'", @"\'");
                ScriptManager.RegisterStartupScript(this, this.GetType(), "456", "ShowEditPost(" +
                    "'" + Convert.ToString(hdnPostUpdateId.Value) + "', " +
                    "'" + dt.Rows[0]["strPostType"].ToString() + "', " +
                    "'" + strPostDesc + "', " +
                    "'" + Convert.ToString(dt.Rows[0]["strPostLink"]) + "', " +
                    "'" + Convert.ToString(dt.Rows[0]["strVideoPath"]) + "', " +
                    "'" + Convert.ToString(dt.Rows[0]["strPhotoPath"]) + "', " +
                    ");", true);
                //ddlPostType.ClearSelection();
    ...
    protected void lstChild_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        ...
        HiddenField hdnimgprofile = e.Item.FindControl("hdnimgprofile") as HiddenField;
        UpdatePanel upda = e.Item.FindControl("upda") as UpdatePanel;
        HiddenField hdnlnkLikes = e.Item.FindControl("hdnlnkLikes") as HiddenField;

        if (hdnCommentLikeId.Value == ViewState["UserID"].ToString())
        {
            hdnlnkLikes.Value = "Unlike";
