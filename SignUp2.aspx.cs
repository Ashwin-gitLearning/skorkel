...
public partial class SignUp2 : System.Web.UI.Page
{
    ...
    protected void GetGmailLogInDetails()
    {
        ...
        catch (WebException ex)
        {
            if (Session["ExternalUserId"] != null)
            {
                ...
                //Add UserID Cookie
                myScrlCookie.Values.Add("UserID", Convert.ToString(Session["ExternalUserId"]));
                myScrlCookie.Values.Add("UserName", ((UserSession.UserInfo)Session["UInfo"]).UserName);
                myScrlCookie.Values.Add("Password", "");
                ...
                if (response.StatusCode.ToString().ToLower() == "ok")
                {
                    ...
                    if (content != null)
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            SendMailGoogleLinkedIn(objRegistration);
                            ...
                            if (ISAPIURLACCESSED == "1")
                            {
								...
								UserURL.Append("uid=");
								UserURL.Append(objRegistration.RegOutId);
								UserURL.Append("&userId=");
								UserURL.Append(objRegistration.UserName);
								UserURL.Append("&password=");
								UserURL.Append(objRegistration.Password);
								UserURL.Append("&firstName=");
								UserURL.Append(objRegistration.FirstName);
								UserURL.Append("&lastName=");
								UserURL.Append(objRegistration.LastName);
    ...
    public void SaveUserDetails()
    {
        ...
        if (dtUser.Rows.Count > 0)
        {
            ...
            //objRegistration.notificationdatetime = DateTime.Now();
            ...
            if (Convert.ToString(ViewState["FromLinkedIn"]) == "1")
            {
                ...
                if (ISAPIURLACCESSED == "1")
                {
					...
					UserURL.Append("registerUser.action?");
					UserURL.Append("uid=");
					UserURL.Append(Convert.ToString(Session["ExternalUserId"]));
					UserURL.Append("&userId=");
					UserURL.Append(dtUser.Rows[0]["UserName"]);
					UserURL.Append("&password=");
					UserURL.Append(dtUser.Rows[0]["Password"]);
					UserURL.Append("&firstName=");
					UserURL.Append(dtUser.Rows[0]["FirstName"]);
					UserURL.Append("&lastName=");
					UserURL.Append(dtUser.Rows[0]["LastName"]);
					UserURL.Append("&userType=STUDENT");
					UserURL.Append("&userContextIds=" + null);
					UserURL.Append("&friendUserIds=" + null);
					UserURL.Append("&lawRelated=" + null);
    ...
    protected void SaveSignup()
    {
        if (txtInstituteName.Text != "")
        {
            ...
            if (txtDegree.Text != "")
            {
                ...
                objDOEdu.strDegree = Validations.validateHtmlInput(txtDegree.Text.Trim().Replace("'", "''"));
            ...
            if (txtInstituteName.Text != "")
            {
                ...
                //Defect #9 Validate HTML Input
                objDOEdu.strInstituteName = Validations.validateHtmlInput(txtInstituteName.Text.Trim().Replace("'", "''"));
            ...
            //objDOEdu.strSpclLibrary = Validations.validateHtmlInput(objDOEdu.strSpclLibrary);
            ...
            if (ISAPIURLACCESSED == "1")
            {
                try
                {
					...
					url.Append("&expId=");
					url.Append(objDO.intOutId);
					url.Append("&designation=");
					url.Append(objDO.strDesignation);
					url.Append("&company=");
					url.Append(objDO.strCompanyName);
					url.Append("&workFrom=");
					url.Append(objDO.dtFromDate);
					url.Append("&workTo=");
					url.Append(CurrentlyWork);
					url.Append("&details=");
					url.Append(objDO.strDescription);
        ...
        signUpModal.Style.Add("display", "none");
    }
    ...
    private void SendMail(DO_Registrationdetails objRegistration)
    {
        try
        {
            ...
            try
            {
                ...
                string emailLink = MailURL + "/SignUp1.aspx" + encript;

                body = body.Replace("{UserName}", objRegistration.FirstName + " " + objRegistration.LastName);
                body = body.Replace("{Content}", "Thank you for creating a Skorkel account. <br/><br/> Your username is your email address which you'll need when you log in. <br/><br/>" +
                        "Please click on the link below to activate your account. <br/><br/>" + "<a href='" + emailLink + "'style='color:#01B7BD; text-decoration: none;cursor:pointer;'>" + emailLink + "</a>" + "<br>");
    ...
    private void SendMailGoogleLinkedIn(DO_Registrationdetails objRegistration)
    {
        try
        {
            string body = null;
            ...
            try
            {
                ...
                FCMNotification.Send("Welcome to Skorkel", "Welcome to Skorkel. Skorkel is a one stop platform for you to - connect with the law student community; research and collaborate; " +
                       "access legal news; access journals and contribute blogs & articles; apply for jobs and internships.", Convert.ToString(objRegistration.RegOutId), MailURL);
                Rmm2.To.Clear();
    ...
    protected void btnAreaExpSave_Click(object sender, EventArgs e)
    {
        if (txtAreaExpert.Text != "")
        {
            ...
            objDO.dtFromDate = DateTime.Now;
            objDO.dtToDate = DateTime.Now;
            //Defect #9 Validations
            objDO.strSkillName = Validations.validateHtmlInput(txtAreaExpert.Text.Trim());
