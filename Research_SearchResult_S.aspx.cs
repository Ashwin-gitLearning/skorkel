...
public partial class Research_SearchResult_S : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
           ...
           // AutoCompleteExtender1.ServicePath = ConfigurationManager.AppSettings["ResearchURL"];
            BindSubjectList();
            ...
            if (Session["strSearchQuery"] != null)
            {
                ...
                ViewState["SortUrl"] = Session["strSearchQuery"].ToString();
                lnkSAvesearch.Enabled = true;
                ViewState["Searchresult"] = "Searchresult";

                lblEnterKeyword.Text = "Saved Search \"" + Session["strSavedSearchTitle"].ToString() + "\"";
                hdnEnterKeyword.Value = lblEnterKeyword.Text;
                lnkSAvesearch.Visible = false;
                divlbls.Visible = true;
                Session.Remove("strSearchQuery");
                Session.Remove("strSavedSearchTitle");
                divsearchHeight.Attributes["class"] = "";
            } else
            {
                divsearchHeight.Attributes["class"] = "researchblock";
            }
    ...
    protected void btnJudges_Click(Object sender, EventArgs e)
    {
        ...
        txtJudgesMember.selectPlaceholder = "Enter the names of the judges";
        if (dtSub.Rows.Count > 0)
        {
            txtJudgesMember.DataSource = dtSub;
            txtJudgesMember.DataValueField = "intRegistrationId";
            txtJudgesMember.DataTextField = "JudgeName";
            txtJudgesMember.RefreshList();
        } else
        {
            txtJudgesMember.DataSource = null;
            txtJudgesMember.RefreshList();
        }
        upJudges.Update();
    }
    ...
    protected void BindSubjectList()
    {
        ...
        txtSubjectList.selectPlaceholder = "Select multiple subjects to add in search";
        if (dtSub.Rows.Count > 0)
        {
        ...
        else
        {
            ...
            txtSubjectList.DataSource = null;
            txtSubjectList.RefreshList();
        }
    ...
    private void Search()
    {
		...
		string[] subJectCateStrs = subJectValues.Select(s => s.Key).ToArray();
		string[] subJectCateValues = subJectValues.Select(s => s.Value).ToArray();
		string subjectInt = "";
		string subjectTexts = "";
		if (subJectCateValues.Count() > 0)
		{
			subjectInt = String.Join(",", subJectCateValues);
			subjectTexts = String.Join(",", subJectCateStrs);
		}

		//List<KeyValuePair<string, string>> judgeValues = txtJudgesMember.GetSelectedValues();
		//string[] judgeTextsArr = judgeValues.Select(s => s.Key).ToArray();
		//string[] judgeIntsArr = judgeValues.Select(s => s.Value).ToArray();
        string judgeInts = hdnMembId.Value;
        string judgeTexts = hdnMembIdText.Value; ;
		//if (judgeTextsArr.Count() > 0)
		//{
		//	judgeInts = String.Join(",", judgeIntsArr);
		//	judgeTexts = String.Join(",", judgeTextsArr);
		//}

		if (judgeInts == "" && subjectInt == "" && ddlYearFT.Text == "Select year" && ddlCourtFT.Text == "Select type of court")
		{
            //expanded = false;
		}
        ...
        //String url = "http://13.127.203.183/SkorkelWeb/skorkelAdvanceSearch.action?title=kumari&searchtype=FREE+TEXT";

        StringBuilder url = new StringBuilder();
        url.Append(APIURL);
        url.Append("skorkelAdvanceSearch.action?");
        url.Append("suid=");
        url.Append(Convert.ToString(ViewState["UserID"]));
        url.Append("&lookInto=");
        url.Append("");
        url.Append("&title=");
        url.Append(Convert.ToString(txtResearch.Text));
        url.Append("&citation=");
        url.Append(Convert.ToString(EqCitatios.Trim()));
        url.Append("&context=");
        url.Append(Convert.ToString(subjectTexts));
        url.Append("&dateFrom=" + null);
        url.Append("&dateTo=" + null);
        url.Append("&judgeName=");
        url.Append(Convert.ToString(judgeTexts));
        url.Append(Convert.ToString(subjectTexts));
        url.Append("&sortCriteriya=");
        url.Append("MR");
        url.Append("&maxResultPerPage=");
        url.Append(50);
        url.Append("&pageIdx=" + null);
        url.Append("&year=");
        url.Append(Convert.ToString(year));
        url.Append("&court=");
        url.Append(Convert.ToString(court));
        url.Append("&reporter=");
        url.Append(Convert.ToString(report));
        url.Append("&volume=");
        url.Append(Convert.ToString(volume));
        url.Append("&cursor=" + null);
        url.Append("&searchtype=");
        url.Append(ddlSelect.SelectedItem.Text);
        url.Append("&pageNo=");
        url.Append(Convert.ToString(pageNo));
        
        //StringBuilder url = new StringBuilder();
        //url.Append(APIURL);
        //url.Append("skorkelAdvanceSearch?");
        //url.Append("title=");
        //url.Append(Convert.ToString(txtResearch.Text));
        //url.Append("&searchtype=");
        //url.Append(ddlSelect.SelectedItem.Text);
        ...
        //      objAPILogDO.strAPIType = "Universal Search";
        ...
        hdnSearchSelect.Value = ddlSelect.SelectedItem.Value;

		if (ddlSelect.SelectedItem.Text == "Free Text")
		{
			string s = txtResearch.Text.Trim() + "," + judgeTexts + "," + year + "," + court;
			if (txtResearch.Text.Trim() != "")
			{
				if (judgeTexts.Trim() != "")
				{
					if (year.Trim() != "")
					{
						if (court.Trim() != "")
						{
							s = txtResearch.Text.Trim() + "," + judgeTexts + "," + year + "," + court;
						}
						else
						{
							s = txtResearch.Text.Trim() + "," + judgeTexts + "," + year;
						}
					...
					else
					{
						if (court.Trim() != "")
						{
							s = txtResearch.Text.Trim() + "," + judgeTexts + "," + court;
						}
						else
						{
							s = txtResearch.Text.Trim() + "," + judgeTexts;
						}
					}
				}
				else
				{
					if (year.Trim() != "")
					{
						if (court.Trim() != "")
						{
							s = txtResearch.Text.Trim() + "," + year + "," + court;
						}
						else
						{
							s = txtResearch.Text.Trim() + "," + year;
						}
					...
					else
					{
						if (court.Trim() != "")
						{
							s = txtResearch.Text.Trim() + "," + court;
						}
						else
						{
							s = txtResearch.Text.Trim();
						}
					}
				}
				string Fstring = s.Length <= 60 ? s : new string(s.Take(60).ToArray()) + "...";
		...
		{
			string s = judgeTexts + ", " + year + ", " + court;
			if (judgeTexts != "")
			{
				s = judgeTexts;
			}
		...
		GetMySavedSearchResult(url.ToString());
		lnkSAvesearch.Visible = true;
        ViewState["Searchresult"] = "Searchresult";
        //CiYear.Value = "Year";
        //CiCourt.Value = "Court";
    ...
    public static List<string> GetCompletionList(string prefixText, int count)
    {
        try
        {
            ...
            url += "autocompleteSearchContent?prefixText=" + prefixText + "&count=" + count;
            using (WebClient wc = new WebClient())
            {
                var content = wc.DownloadString(url);
                dynamic jsonResponse = JsonConvert.DeserializeObject(content);
                List<string> resp = jsonResponse.responseJson.ToObject<List<string>>();
                if (resp.Count > 10)
                    resp = resp.GetRange(0, 10);
                return resp;
            }
        }
       catch(Exception ex)
        ...
        if (my.responseJson != null)
        {
            try
            {
                ...
                else
                {
                    ...
                    lstSearchResult.DataBind();
                    divsearchHeight.Style.Add("min-height", "550px");
                    dtResearch = new DataTable();
                    pLoadMore.Style.Add("display", "none");
                    lblNoMoreRslt.Visible = true;
    ...
    protected void FetchNextRows()
    {
		...
		StringBuilder url = new StringBuilder();
		url.Append(APIURL);
		url.Append("skorkelSearchHits.action?");
		url.Append("cursorId=");
		url.Append(CursorId);
    ...
    protected void lnkPopupOK_Click(object sender, EventArgs e)
    {
        if (ViewState["Searchresult"] != null)
        {
            ...
            ScriptManager.RegisterStartupScript(this, this.GetType(), "starScripts1", "$('#successSaveSearch').show();", true);
        }
        else
        {
    ...
    {
        if (hdnAvoid.Value == "true") { hdnAvoid.Value = "false"; }
        else Search();
    }
}
