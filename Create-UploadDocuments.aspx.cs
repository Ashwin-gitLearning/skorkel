...
public partial class Create_UploadDocuments : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //ddUploader1.URL = "handler/UploadDocument.ashx";
    ...
    protected void GetDocsDetailsForEdit()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            ddUploader1.SetValues(dt.Rows[0]["strDocName"].ToString(), dt.Rows[0]["strFilePath"].ToString());
       ...
       //     string imgPathPhysical = Server.MapPath("~/UploadDocument/" + hdnUploadFile.Value.ToString());
            ...
            if (dtSub.Rows.Count > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        string str = dr["intCategoryId"].ToString();
                        if (!string.IsNullOrEmpty(str))
                        {
                            selectedValues.Add(str);
                        }
                    }
                    lstSubjCategory.SetValues(selectedValues.ToArray());
                }
            }
        }
    }
   ...
   //     lblDocName.Visible = false;
    ...
    protected void LstSubjCategory_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        if (e.CommandName == "Subject Category")
        {
                ...
                if (dtContent.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtContent.Rows.Count; i++)
                    {
                        ...
                        if (contains == true)
                        {
                            ...
                            SubLi.Style.Add("border", "1px solid #ddd");
            ...
            if (dtsub.Rows.Count > 0)
            {
                SubLi.Style.Add("background", "none repeat scroll 0 0 #949898 !important");
            ...
            {
                SubLi.Style.Add("background", "#949898");
                SubLi.Style.Add("border", "1px solid #ddd");
                SubLi.Style.Add("color", "#646161");
            }
        }
        ...
        if (ViewState["SubjectCategory"] != null)
        {
            ...
            if (dtsub.Rows.Count > 0)
            {
                for (int i = 0; i < dtsub.Rows.Count; i++)
                {
                    ...
                    {
                        SubLi.Style.Add("background", "none repeat scroll 0 0 #6C7373 !important");
                        SubLi.Style.Add("color", "#FFFFFF !important");
                        SubLi.Style.Add("text-decoration", "none !important");
                        lnkCatName.ForeColor = System.Drawing.Color.White;
                        lnkCatName.Style.Add("color", "#FFFFFF !important");
        ...
        if (ViewState["SaveSubCatname"] != null)
        {
            ...
            if (dtsub.Rows.Count > 0)
            {
                for (int i = 0; i < dtsub.Rows.Count; i++)
                {
                    if (hdnSubCatId.Value == dtsub.Rows[i]["intCategoryId"].ToString())
                    {
                        SubLi.Style.Add("background", "none repeat scroll 0 0 #797F7F !important");
                        SubLi.Style.Add("color", "#FFFFFF !important");
                        SubLi.Style.Add("text-decoration", "none !important");
                        lnkCatName.ForeColor = System.Drawing.Color.White;
                        lnkCatName.Style.Add("color", "#FFFFFF !important");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        fname = ddUploader1.UploadedFileName;
        if (fname != "")
        {
            ...
            List<KeyValuePair<string,string>> dtsub = lstSubjCategory.GetSelectedValues();
            //dtsub = (DataTable)ViewState["SubjectCategory"];
            if (dtsub.Count > 0)
            {
                if (ViewState["DocIdEdit"] != null && ViewState["DocIdEdit"].ToString().Trim() != "0")
            ...
            foreach (KeyValuePair<string,string> dr in dtsub)
            {
                if (ViewState["DocIdEdit"] != null && ViewState["DocIdEdit"].ToString().Trim() != "0")
                {
                ...
                objDoProDocs.intSubjectCategoryId = Convert.ToInt32(dr.Value);
                objDoProDocs.intAddedBy = Convert.ToInt32(ViewState["UserID"]);
            ...
            ScriptManager.RegisterStartupScript(this, this.GetType(), "doup", "OverlayBody();", true);

        }
       
        else
        {
               divDocsSuccess.Style.Add("display", "none");
            lblMessage.Text = ddUploader1.ErrorMesssage;// "Please Select Document.";
